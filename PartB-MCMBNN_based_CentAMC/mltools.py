...
def plot_confusion_matrix(cm, title='Confusion matrix', cmap=plt.get_cmap("Blues"), labels=[],save_filename=None):
    ...
    plt.xticks(tick_marks, labels, rotation=50,size=9)
    plt.yticks(tick_marks, labels,size=9)
        ...
        for j in range(len(tick_marks)):
            if i!=j:
                ...
                if int(np.around(cm[i,j]*100))==100:
                    text=plt.text(j,i,int(np.around(cm[i,j]*100)),ha="center",va="center",fontsize=8,color='white')
                else:
                    text=plt.text(j,i,int(np.around(cm[i,j]*100)),ha="center",va="center",fontsize=8,color='white')
